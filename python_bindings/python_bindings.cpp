   ...
   public:
    ...
    inline void add_dataset(py::buffer& input_data_buffer) {
        ...
        if (buffer.ndim == 2) {
            ...
            std::runtime_error("Input array must be 2 dimensional");
    ...
    inline void print_representation(elementID const member_index) { alg_hhsp->print_representation(member_index); }
    ...
    // Create the index
    inline void create_index(std::vector<float> radius_vector) {
        printf("Creating the Index With Radii: {");
        for (auto rad : radius_vector) printf("%.4f,",rad);
        printf("}\n");

        alg_hhsp->create_index(radius_vector);
        return;
    }

    inline void validate_index() {
        alg_hhsp->validate_index();
        return;
    }

    // Take Numpy Buffer as Input: allows for pointer to original dataset storage
    inline py::list Hierarchical_HSP_Test(py::buffer& query_data_buffer) {
        // process the incoming data
        py::array_t<float, py::array::c_style | py::array::forcecast> items(query_data_buffer);
        auto buffer = items.request();
        size_t rows, features;
        ...
        hsp_neighbors.resize(num_queries);
        for (int i = 0; i < num_queries; i++) {
            alg_hhsp->Hierarchical_HSP_Test((float*)items.data(i), hsp_neighbors[i]);
        }
...
PYBIND11_MODULE(HHSP, handle) {
    ...
    py::class_<HHSP_Bindings>(handle, "HHSP")
        ...
        .def("create_index", &HHSP_Bindings::create_index, py::arg("radius vector"))
        .def("validate_index", &HHSP_Bindings::validate_index)
        .def("Hierarchical_HSP_Test", &HHSP_Bindings::Hierarchical_HSP_Test, py::arg("query data"));
