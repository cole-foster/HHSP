...
struct Pivot {
   public:
    Pivot(){};
    Pivot(unsigned int index): _index(index) {};
    Pivot(unsigned int index, float const radius) : _index(index), _radius(radius){};
    Pivot(unsigned int index, float const radius, int const level) : _index(index), _radius(radius), _level(level) {};
    ~Pivot(){};
    ...
    inline void addChild(unsigned int childIndex, float const distance) {
        _childCount++;
        _pivotDomain.push_back(Pivot(childIndex));  // initialize point as a pivot with 0 radius
        if (distance > _maxChildDistance) {
            _maxChildDistance = distance;
        }
    }
